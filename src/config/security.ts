...
export const SECURITY_CONFIG = {
  ...
  emailService: {
    ...
    templates: {
      userNotification:
        process.env.REACT_APP_EMAILJS_USER_TEMPLATE || "template_quiz_enhanced",
      ownerNotification:
        process.env.REACT_APP_EMAILJS_OWNER_TEMPLATE ||
  ...
  captcha: {
    siteKey:
      ...
      "6Ld7ZTYUAAAAAGgBvCrSeiQrUBLw55jP8hetKuer",
...
export class SecurityUtils {
  ...
  static isValidName(name: string): boolean {
    return (
      name.length >= SECURITY_CONFIG.validation.name.minLength &&
  ...
  static isValidQuizScore(score: number): boolean {
    return (
      score >= SECURITY_CONFIG.validation.quizScore.min &&
  ...
  static isValidMessage(message: string): boolean {
    return (
      message.length >= SECURITY_CONFIG.validation.message.minLength &&
