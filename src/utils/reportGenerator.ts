...
export class ReportGenerator {
  ...
  private addQuizResults(data: QuizResultData) {
    ...
    this.doc.setFontSize(14);
    this.doc.setFont("helvetica", "bold");
    this.doc.text(`${scorePercentage}%`, barX + barWidth / 2, barY + 13, {
      align: "center",
    });
    ...
    this.currentY += 8;
    this.doc.text(
      `Performance Level: ${this.getPerformanceLevel(scorePercentage)}`,
      this.margin,
      this.currentY,
    );
    this.currentY += 8;
  ...
  private addDetailedAnalysis(data: QuizResultData) {
    ...
    lines.forEach((line, index) => {
      this.doc.text(
        line,
        this.margin + 10,
        this.currentY + 15 + index * lineHeight,
      );
    });
  ...
  private addPersonalRecommendations(data: QuizResultData) {
    ...
    this.doc.text(
      "Personalized recommendations will be added here.",
      this.margin + 10,
      this.currentY + 20,
    );
    this.doc.text(
      "This section will contain specific advice tailored to your results.",
      this.margin + 10,
      this.currentY + 35,
    );
