...
const OwnerDashboard: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const validResults = storedResults.filter((result: any) =>
            result &&
    ...
    useEffect(() => {
        ...
        if (searchTerm) {
            const sanitizedSearch = SecurityUtils.sanitizeInput(searchTerm).toLowerCase();
    ...
    return (
        <div className="owner-dashboard">
            <div className="dashboard-header glass-card">
                ...
                <p>{t.ownerDashboard?.subtitle || 'View and download all quiz results'}</p>
            </div>
            ...
            <div className="dashboard-controls glass-card">
                <div className="search-filters">
                    <input
                        type="text"
                        placeholder={t.ownerDashboard?.searchPlaceholder || 'Search by email, quiz, or result...'}
                        value={searchTerm}
                    ...
                    >
                        <option value="all">{t.ownerDashboard?.allQuizzes || 'All Quizzes'}</option>
                ...
                <div className="dashboard-stats">
                    <div className="stat-item">
                        ...
                        <span className="stat-label">{t.ownerDashboard?.totalResults || 'Total Results'}</span>
                    </div>
                    <div className="stat-item">
                        <span className="stat-number">{filteredResults.length}</span>
                        <span className="stat-label">{t.ownerDashboard?.filteredResults || 'Filtered Results'}</span>
                    </div>
                    <div className="stat-item">
                        <span className="stat-number">{getUniqueQuizNames().length}</span>
                        <span className="stat-label">{t.ownerDashboard?.uniqueQuizzes || 'Unique Quizzes'}</span>
                    </div>
                </div>
            </div>
            ...
            <div className="results-container">
                {filteredResults.length === 0 ? (
                    <div className="no-results glass-card">
                        ...
                        <p>{t.ownerDashboard?.noResultsSubtitle || 'Try adjusting your search or filters'}</p>
                    </div>
                ) : (
                        ...
                        <div key={result.id} className="result-card glass-card">
                            <div className="result-header">
                                ...
                                <div className="result-score">
                                    ...
                                    <span className="score-label">{t.ownerDashboard?.score || 'Score'}</span>
                                </div>
                            </div>
