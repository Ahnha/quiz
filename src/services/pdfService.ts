...
export class PDFService {
  ...
  private static async addHeader(doc: jsPDF, data: PDFData): Promise<void> {
    ...
    const subtitle =
      data.language === "en"
        ? "Personalized Skin Care Report"
        : "Raport Personalizat de Îngrijire a Pielii";
  ...
  private static addUserInfo(doc: jsPDF, data: PDFData): number {
    ...
    const reportFor =
      data.language === "en" ? "Report for: " : "Raport pentru: ";
  ...
  private static addQuizResultSection(
    ...
    doc.setTextColor(156, 39, 176);
    const testResult =
      data.language === "en" ? "Test Result:" : "Rezultatul Testului:";
    ...
    if (data.quizResult && typeof data.quizResult.text === "object") {
      resultText =
        data.language === "en"
          ? data.quizResult.text.en
          : data.quizResult.text.ro;
    }
  ...
  private static addScientificStudiesSection(
    doc: jsPDF,
    data: PDFData,
    startY: number,
  ): number {
  ...
  private static addRecommendationsSection(
    ...
    if (
      ...
      data.skinRecommendation.recommendedProducts
    ) {
      doc.setFontSize(10);
      this.setFont(doc, "normal");
      doc.setTextColor(50, 50, 50);

      data.skinRecommendation.recommendedProducts.forEach(
  ...
  private static addSkinTypeSection(
    doc: jsPDF,
    data: PDFData,
    startY: number,
  ): number {
    ...
    if (!skinRec || !skinRec.title || !skinRec.description) {
      ...
      doc.setTextColor(50, 50, 50);
      doc.text(
        "Personalized skin care recommendations based on your quiz results.",
        20,
        currentY,
      );
      currentY += 20;
    ...
    const descLines = doc.splitTextToSize(
      skinRec.description || "Personalized recommendations for your skin type.",
      170,
    );
    doc.text(descLines, 20, currentY);
    currentY += descLines.length * 5 + 10;
    ...
    // Daily routine
    if (
      skinRec.dailyRoutine &&
      Array.isArray(skinRec.dailyRoutine) &&
      skinRec.dailyRoutine.length > 0
    ) {
      doc.setFontSize(13);
      this.setFont(doc, "bold");
      doc.setTextColor(100, 100, 100);
      const dailyRoutine =
        data.language === "en" ? "Daily Routine:" : "Rutina Zilnică:";
    ...
    if (
      skinRec.recommendedProducts &&
      Array.isArray(skinRec.recommendedProducts) &&
      skinRec.recommendedProducts.length > 0
    ) {
      doc.setFontSize(13);
      this.setFont(doc, "bold");
      doc.setTextColor(100, 100, 100);
  ...
  private static addAgingSection(
    doc: jsPDF,
    data: PDFData,
    startY: number,
  ): number {
    ...
    if (!agingRec || !agingRec.title || !agingRec.description) {
      ...
      doc.setTextColor(50, 50, 50);
      doc.text(
        "Personalized anti-aging recommendations based on your quiz results.",
        20,
        currentY,
      );
      currentY += 20;
    ...
    doc.setTextColor(50, 50, 50);
    const descLines = doc.splitTextToSize(
      agingRec.description || "Personalized anti-aging recommendations.",
      170,
    );
    doc.text(descLines, 20, currentY);
    currentY += descLines.length * 5 + 10;
    ...
    // Anti-aging routine
    if (
      agingRec.antiAgingRoutine &&
      Array.isArray(agingRec.antiAgingRoutine) &&
      agingRec.antiAgingRoutine.length > 0
    ) {
      doc.setFontSize(13);
      this.setFont(doc, "bold");
      doc.setTextColor(100, 100, 100);
      const antiAgingRoutine =
        data.language === "en"
          ? "Anti-Aging Routine:"
          : "Rutina Anti-Îmbătrânire:";
  ...
  private static addLifestyleSection(
    doc: jsPDF,
    data: PDFData,
    startY: number,
  ): number {
    ...
    if (
      skinRec &&
      skinRec.lifestyleTips &&
      Array.isArray(skinRec.lifestyleTips) &&
      skinRec.lifestyleTips.length > 0
    ) {
      doc.setFontSize(13);
      this.setFont(doc, "bold");
      doc.setTextColor(100, 100, 100);
    ...
    if (
      agingRec &&
      agingRec.lifestyleChanges &&
      Array.isArray(agingRec.lifestyleChanges) &&
      agingRec.lifestyleChanges.length > 0
    ) {
      doc.setFontSize(13);
      this.setFont(doc, "bold");
      doc.setTextColor(100, 100, 100);
  ...
  private static addIngredientsSection(
    doc: jsPDF,
    data: PDFData,
    startY: number,
  ): number {
    ...
    if (
      skinRec &&
      skinRec.ingredients &&
      Array.isArray(skinRec.ingredients) &&
      skinRec.ingredients.length > 0
    ) {
      doc.setFontSize(13);
      this.setFont(doc, "bold");
      doc.setTextColor(100, 100, 100);
    ...
    if (
      skinRec &&
      skinRec.avoidIngredients &&
      Array.isArray(skinRec.avoidIngredients) &&
      skinRec.avoidIngredients.length > 0
    ) {
      doc.setFontSize(13);
      this.setFont(doc, "bold");
      doc.setTextColor(100, 100, 100);
  ...
  private static addAdditionalNoteSection(
    doc: jsPDF,
    startY: number,
    language: "ro" | "en",
  ): number {
    ...
    doc.setTextColor(100, 100, 100);
    doc.text(
      language === "ro" ? "Cum să folosești:" : "How to use:",
      20,
      currentY,
    );
    currentY += 8;
  ...
  private static addFooter(doc: jsPDF, data: PDFData): void {
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      const footerText =
        data.language === "en"
          ? "Skin Studio - Personalized Skin Care Report"
          : "Skin Studio - Raport Personalizat de Îngrijire a Pielii";
  ...
  public static async generatePDF(data: PDFData): Promise<Blob> {
    try {
      ...
      currentY = this.addScientificStudiesSection(
        this.doc,
        data,
        currentY + 15,
      );
      ...
      currentY = this.addAdditionalNoteSection(
        this.doc,
        currentY + 10,
        data.language || "ro",
      );
      ...
      if (
        data.skinRecommendation &&
        data.skinRecommendation.professionalAdvice
      ) {
        if (currentY > 250) {
          this.doc.addPage();
          currentY = 30;
        ...
        const adviceLines = this.doc.splitTextToSize(
          data.skinRecommendation.professionalAdvice ||
